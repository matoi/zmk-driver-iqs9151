...
クロスパッドジェスチャーは、この確定済みの出力を**別の用途に変換する**機能
（例: 1F のカーソル移動 → Ctrl + wheel によるピンチズーム、
2F のスクロール → ドラッグ＆ドロップ用のカーソル移動）であるため、
仮に input-processor で処理しようとすると、通常処理で既に行われた変換を
その都度逆変換してから再変換する必要が生じ、かえって複雑になる。
