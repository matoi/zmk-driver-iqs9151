...
また、このドライバは 2 本指スクロール等も同様にドライバ内で処理しており、
input-processor に届く時点で `REL_X`/`REL_Y` か `REL_WHEEL`/`REL_HWHEEL` かは
確定済み。クロスパッドもこの構造に合致している。
...
不採用理由:
- **2F 側の通常処理はスクロール**: ホールド中に 2F 側が必要なのはカーソル移動だが、
