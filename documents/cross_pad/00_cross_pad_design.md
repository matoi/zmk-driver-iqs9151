...
**原因（未確定: ボタン down→up の繰り返し）:**

「片側を操作し続けると down→up が連続する」現象の原因は未確定。
考えられる仮説:

1. **undecided のキャンセル・再突入の繰り返し**: peer_fc が揺れて
   undecided → キャンセル（通常処理再開、BTN press）→ 再び undecided（cleanup、BTN release）
   を繰り返す。ただし、指が静止している場合は fc が揺れにくいため、この仮説の妥当性は疑問
2. **通常処理の内部状態が cleanup 後も影響を残している**: `cleanup_normal` で
   リセットした後に通常処理が再開された際、何らかの状態遷移がボタンイベントを生む
3. **cross-pad ジェスチャーの終了・再開が繰り返されている**: hold_continuing 等の
   条件が一瞬 false になり、ジェスチャーが終了→通常処理→再度 cross-pad 開始のサイクル

実機でのログ取得（`cross-pad: undecided start/restart/cancelled` 等のパターン確認）
による原因の特定が必要。
