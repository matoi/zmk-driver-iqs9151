...
| peer からの fc 定期再送（ハートビート） | BLE トラフィック増加、実装の複雑化。稀な問題に対して過剰 |

**根本原因:** 「peer が本当にまだ触っている」と「peer は離したが通知が欠落した」を
区別する手段がない。確実な解決にはハートビート等の冗長通信が必要だが、
現時点では稀な発生頻度と自己回復性を考慮し、既知の制限として許容する。
