...
### 案3: セントラル一元処理（制約 1・2 を外した場合）

上記の制約を両方外し、ペリフェラルのトラックパッドを cross-pad 有効時に
...
### 案4: ペリフェラルをジェスチャー指定、セントラルを操作処理に分離（制約 2 のみを外した場合）
