...
### 案6: ペリフェラルをジェスチャー指定、セントラルを操作処理に分離（制約 2 のみを外した場合）
...
案5（フルスクラッチ）と比べて、通常操作の処理を書き直す必要がない分
